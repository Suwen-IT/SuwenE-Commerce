   ...
   public class UpdateBasketItemCommandHandler : IRequestHandler<UpdateBasketItemCommandRequest, ResponseModel<BasketDto>>
   {
        ...
        public async Task<ResponseModel<BasketDto>> Handle(UpdateBasketItemCommandRequest request, CancellationToken cancellationToken)
        {
            ...
            var product = await _unitOfWork.GetReadRepository<Product>()
                .GetByIdAsync(basketItem.ProductId, tracking: false);
            ...
            if (basketItem.ProductAttributeValueId.HasValue)
            {
                ...
                var pav = await _unitOfWork.GetReadRepository<ProductAttributeValue>()
                    .GetByIdAsync(pavId, tracking: false);
            ...
            await _unitOfWork.GetWriteRepository<BasketItem>().UpdateAsync(basketItem);

            var success = await _unitOfWork.SaveChangesBoolAsync();

            if (!success)
                return new ResponseModel<BasketDto>("Sepet ürünü güncellenirken hata oluştu.", 500);
